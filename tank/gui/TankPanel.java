...
public class TankPanel extends JPanel {
	...
	public TankPanel(){
		...
		begin.setBounds(new Rectangle(200,50,79,41));
		this.add(begin);
	...
	private void initComponent(){
		...
		text.setText("Click the button to play again");
		...
		ImageIcon selfIcon = new ImageIcon("self_icon.png");
		hpIcon.setIcon(selfIcon);
		this.add(hpIcon);
		
		btn = new JButton("Continue round1");
		...
		icon = new ImageIcon("../image/myTank_r.png");
	...
	public void paintComponent(Graphics g) {
        ...
        for(int i=1; i-1<scene.getWaitedTanks().size();i++){
        	...
        	Cell c = new Cell(x,y,Mapelements.ELEMENT_TYPE_MINITANK);
        	drawObjects(g,c);
        }
	...
	private void drawMap(Graphics g){
		 ...
		 for (int x = 0; x < map.length; x++) {
            for (int y = 0; y < map[0].length; y++) {
            	...
            	g.drawImage(img, y * Mapelements.CS, x * Mapelements.CS, this);
            }
		 }
	}
